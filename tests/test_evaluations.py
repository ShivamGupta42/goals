...
def test_goal_scenarios_are_supported_by_current_mode_a(monkeypatch, tmp_path: Path) -> None:
    ...
    assert any(
        "mandatory_external_review_gate" in result.supported_capabilities for result in results
    )
    assert all(
        "mandatory_external_review_gate" not in result.planned_capabilities for result in results
    )
    ...
    assert any("cross_project_memory_sync" in result.supported_capabilities for result in results)
    assert all("cross_project_memory_sync" not in result.planned_capabilities for result in results)
...
def test_use_case_coverage_reports_broad_current_and_future_fit(
    ...
    assert "mandatory_external_review_gate" in high_stakes.supported_capabilities
    assert "professional_boundary_templates" not in high_stakes.planned_capabilities
    assert "mandatory_external_review_gate" not in high_stakes.planned_capabilities
    ...
    operations = next(case for case in report.cases if case.use_case_id == "operations-process")
    assert "handoff_owner_registry" in operations.supported_capabilities
    assert "handoff_owner_registry" not in operations.planned_capabilities
...
def test_self_check_summarizes_all_evaluation_suites(monkeypatch, tmp_path: Path) -> None:
    ...
    assert report.next_slices[0] == "Explore planned capability: optional calendar context"
