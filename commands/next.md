...
3. Make the change, then **prove it by execution, not description**. For each
   acceptance criterion AND each load-bearing assumption, write a runnable check
   that fails if it's wrong — push it at the boundary/environment conditions it
   could break under (limits, signs, emptiness, scale, clock/locale, storage or
   network failing). List them in the evidence `verifications` (each `covers` a
   criterion or an assumption id). Run `goals phase evidence <PHASE> --file
   <evidence.json>`, then `goals phase verify <PHASE>` — the engine runs your
   checks and records the real results; you cannot pass one yourself. Fix until
